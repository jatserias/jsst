...
/*
 * PS HMM basic implementation
 * 
 * 
 */
public class PS_HMM extends Chain{
	...
	public PS_HMM(final SstTagSet tagset) {
		super();
		this.tagset=tagset;
		it = 0; 
		only_active = false; 
    ...
    //@Overrite
	  ...
	  * @param wc
	  */
	public void init(FeatureEncoder fenc, boolean no_spec, double wc){
	  ...
	  if( !no_spec ) {
		...
		//?Constructor 
		 ...
		 // bigrams begin
	     init_phi_sf(phi_s,fenc,true,tagset);         
	     
	     // bigrams end
	     init_phi_sf(phi_f,fenc,false,tagset);       
	  ...
	  phi_b = new Fs[k][k];
	  init_phi_b(k,phi_b,fenc,tagset);               // std transitions
	 ...
	 */
	 ...
	 @Override 
	 protected double get_word_contribution( int[]  x, int j, int pos ){
	 ...
	 void train( TrainingData trainset, Set<Integer> Index 
		  ...
		  for ( int i = 0; i < N; ++i ){
		   ...
		   // boolean updated_s = train_on_s( W[OUT[i]], T[OUT[i]]);
		    TrainingInstance t = trainset.getTrainingExample(OUT[i]);
	 ...
	 * @throws IOException 
	 */
	boolean  train_on_s( TrainingInstance e) throws IOException {
		int[][] W = e.W; 
		int[] T = e.T;
		 try {
 ...
 if( !active || act_phi_w.get(l).contains(feat)) {
   ...
   //System.err.println("update:"+phi_w.get(l).get( feat ).dump());
	 ...
	 protected void update_word_feats( int[] w, int T, int U ){
		  int _X_ = w.length;
		  for ( int s = 0; s < _X_; ++s ){
		    update_feature_val( phi_w, U, w[s], -UPDATE_STEP, only_active, active_phi_w );
		    update_feature_val( phi_w, T, w[s], UPDATE_STEP, only_active, active_phi_w );
		  }
		}
