...
/**
 * @TODO A test unit is needed to verify results are comparable with the C version
 ...
 * @TODO maybe split tagger (has a model) and learner (build a model)
 * @TODO add a function to return the undecoded results
 * 
 * @author jordi
 *
 */
public class SttTagger implements TaggerAPI {
	private static final String[] EMPTY_STRING_ARRAY = new String[0];
	private static final int[] EMPTY_INT_ARRAY = new int[0];
	private static final String ALL_LABEL = "ALL";

	/// Should this be part of the tagger ??
	public TrainingFormatReader freader;
	...
	/// Constructor for tagging
		public SttTagger(FeatureEncoder fe, final FeatureBuilder fb, final ModelDescription model, boolean ergodic) throws Exception {
		 ...
		 * @throws Exception
		 */
		public SttTagger(FeatureEncoder fe,FeatureBuilder fb2, SstTagSet tagset, boolean ergodic) throws Exception {
	...
	///@TODO fix this method.  it returns an instance of an ergodic or non ergodic HMM 
	private static PS_HMM newPSHMM(FeatureEncoder fe,FeatureBuilder fb, SstTagSet tagset, boolean ergodic) {
		if((tagset.mode==tagmode.POS) && !ergodic) {
			logger.warn("Ignoring No ergodic flag in POS mode");
			ergodic=true;
		}
		if(ergodic) return new PS_HMM(tagset);
		else {
			return new PS_HMM_NoErgodic(fe,tagset);
		}
	}
	...
	private void minit(FeatureEncoder fe, final FeatureBuilder fb, final SstTagSet tagset, boolean ergodic) throws Exception {
		...
		mode=tagset.mode;
		ps_hmm = newPSHMM(fe,fb,this.tagset,ergodic);
		et = new ET_BIO();
 ...
 */
	protected void loadLabels() throws IOException {
		...
		for(String input: tagset.getListLabel()) {
			//TODO NOT USE 0
			try {
			   if(   // what if we have label ) in POS mode
					   input.compareTo("0")!=0 && mode==SttTagger.tagmode.BIO) 
				   
				   LABELS.add(input.substring(2));
	     	}
			catch(Exception e) {				
				throw new RuntimeException("Unparsable label:"+input+" in "+mode+" mode\n BIO label should be 0 or start with B- or I- prefix"+e.getMessage());
			}
	 ...
	 * @return
	 */
	public static  final  int[][] encode(final FeatureVector[] O_str, final boolean secondorder, FeatureEncoder fe) {
   			...
   			if (secondorder){
   				for (int r = j+1; r < n; ++r){
   					...
   					if (a.compareTo(b)>0) //?? a< b
   						O_i[r]=(int) fe.encode(""+a+"-"+b);
   					else 
   						O_i[r]=(int) fe.encode(""+b+"-"+a);
   				}
   			}
   		}
   		O[i]=O_i;
	...
	public int[] innerTagging(final FeatureVector[] res) {
		int[][] resint = SttTagger.encode(res , model.secondOrder,fe);
	...
	public void eval_light(String traindata,String testdata, String encoding, FeatureBuilder fb, SstTagSet tagsetname,boolean secondorder, int T, int CV,Boolean R, Double ww,String thetafile) throws Exception{
		...
		eval(ID,ID2,D,D2,G,G2,T,CV,description,mode,ww,thetafile);
		

	}

	/**
	 * Evaluation with fixed training and test repeated CV times
	 * 
	 * @param traindata
	 * @param testdata
	 * @param tagsetname
	 * @param secondorder
	 * @param T
	 * @param CV  number of times to repeat experiment (different training order but train/test are the same)
	 * @param mode
	 * @param R
	 * @param ww
	 * @param thetafile
	 * @throws Exception 
	 */ 
	public void eval_light(TrainingData traindata, TrainingData testdata, FeatureBuilder fb, SstTagSet tagset,boolean secondorder, int T, int CV 
			//, Boolean R, 
			, Double ww
			//,String thetafile
			) throws Exception{
		...
		trainsuff = traindata.getName();//new File(traindata.getName()).getName();
		testsuff = testdata.getName(); //new File(testdata.getName()).getName();
		tagsetsuff = tagsetname; //new File(tagsetname).getName();
	...
	/**
	 * loads data from dname into D, G , ID
	 * @param dname
	 * @param D
	 * @param G
	 * @param ID
	 ...
	 * @throws IOException
	 */
	 void load_data(String dname, String encoding, //TrainingFormatReader freader,
...
//			 System.err.println("Features loaded tagger: "+fe.esize());
			if(te!=null) {
 ...
 */
	...
	public void eval(
		...
		TrainingData cvTrainData= new  TrainingDataSST(D,G,ID);
		TrainingData cvTestData= new  TrainingDataSST(D2,G2,ID2);
		eval(cvTrainData,cvTestData, T, CV,descr,mode,ww);//,ww, thetafile);
	}
	...
	public void eval(
		...
		//@JAB implementation
		List<List<Double> > CV_T_ALL = new Vector<List<Double> >();
		List<List<List<Double> > > CV_POS_RES = new  Vector<List<List<Double> > >();
		...
		for (int cv = 0; cv < CV; ++cv){
			...
			List<List<Double> > POS_RES= new Vector<List<Double> >();

			PS_HMM ps_hmm_cv= newPSHMM(fe,fb,tagset,ergodic);
	 ...
	 **/
	public void train(TrainingData train,int T,String modelname, FeatureDecoder mfd) throws Exception{
		...
		ModelFormatReader.SaveModel(ps_hmm,modelname,mfd);
	}
